...
extension BaseEffect {
    func compileShader(shaderName: String, shaderType: GLenum) -> GLuint {
        ...
        do {
            ...
            var shaderCString: UnsafePointer<GLchar>? = shaderString.utf8String! as UnsafePointer<GLchar>
            glShaderSource(
                shaderHandle,
                GLsizei(1),
                &shaderCString,
    ...
    func compile(vertexShader: String, fragmentShader: String) {
        let vertexShaderName = self.compileShader(shaderName: vertexShader, shaderType: GLenum(GL_VERTEX_SHADER))
        let fragmentShaderName = self.compileShader(shaderName: fragmentShader, shaderType: GLenum(GL_FRAGMENT_SHADER))
