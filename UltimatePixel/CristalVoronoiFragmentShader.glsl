...
vec2 hash22(vec2 p) {
    ...
    return sin( p*6.2831853 + globalTime )*.3 + .5;
...
float m(vec3 p){
    ...
    float h = heightMap(p); // texture2D(iChannel0, p.xy/2.).x; // Texture work too.
...
void mainImage(out vec4 c, vec2 u){
    ...
    vec3 r = normalize(vec3(u - vec2(resX,resY)*.5, resY)),
    o = vec3(0), l = o + vec3(0, 0, -1);
    
    // Rotate the canvas. Note that sine and cosine are kind of rolled into one.
    vec2 a = sin(vec2(1.570796, 0) + globalTime/8.); // Fabrice's observation.
    ...
    if(t<FAR){
        ...
        // texture a bit.
        vec3 tx = vec3(0.1,0.3,0.9);// tex3D(iChannel0, (p*2. + hm*.2), n);
